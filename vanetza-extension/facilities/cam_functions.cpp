...
void copy(const vanetza::facilities::PathHistory& ph, BasicVehicleContainerLowFrequency& container)
{
    ...
    const vanetza::facilities::PathPoint& ref = ph.getReferencePoint();
    std::size_t path_points = 0;

    for (const vanetza::facilities::PathPoint& point : concise_points) {
